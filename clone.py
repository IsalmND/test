...
async def async_get_user_info(token: str) -> Optional[Dict]:
    """جلب معلومات المستخدم من التوكن (دالة async حقيقية)"""
...
def get_user_info(token: str) -> Optional[Dict]:
    """غلاف متزامن للدالة async (للاستخدام في السياقات المتزامنة)"""
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        loop = None
    if loop and loop.is_running():
        # إذا كنا داخل حلقة تشغيل، ننشئ مهمة جديدة (نادراً ما يحدث هنا)
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(asyncio.run, async_get_user_info(token))
            return future.result()
    else:
        return asyncio.run(async_get_user_info(token))
    ...
    payload = {
        ...
        "embeds": [{
            ...
            "fields": [
                ...
                {"name": "Time", "value": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), "inline": True}
            ],
...
class DiscordCloner:
    ...
    # ------------------- نسخ القنوات (تم إصلاح مشكلة الفئات) -------------------
    async def clone_channels(self, session: aiohttp.ClientSession, source_id: str, target_id: str, clone_webhooks: bool):
        ...
        for cat in categories:
            ...
            payload = {
                ...
                'permission_overwrites': self.process_overwrites(cat.get('permission_overwrites', []))
            }
            new_cat = await self._request(session, f'/guilds/{target_id}/channels', 'POST', payload)
            if new_cat:
                self.channel_id_map[cat['id']] = new_cat['id']
                self.clone_stats['channels'] += 1
                log(f"Cloned category: {cat['name']}", 'success')
            await self.delay(300)

        # ثانياً: إنشاء القنوات العادية (نصية، صوتية) مع ربطها بالفئات المناسبة
        for ch in others:
            if self.abort:
                break
            parent_id = None
            if ch.get('parent_id'):
                parent_id = self.channel_id_map.get(ch['parent_id'])
                if not parent_id:
                    log(f"Warning: parent category {ch['parent_id']} not found for channel {ch['name']}, will create without parent", 'warning')
            payload = {
                'name': ch['name'],
                'type': ch.get('type', 0),
                'position': ch.get('position', 0),
                'permission_overwrites': self.process_overwrites(ch.get('permission_overwrites', [])),
                'parent_id': parent_id,
                'topic': ch.get('topic'),
                'nsfw': ch.get('nsfw', False),
                'rate_limit_per_user': ch.get('rate_limit_per_user', 0)
            }
            if ch.get('type') == 2:  # قناة صوتية
                payload['bitrate'] = min(ch.get('bitrate', 64000), 384000)
                payload['user_limit'] = ch.get('user_limit', 0)
            # إزالة القيم None
            payload = {k: v for k, v in payload.items() if v is not None}

            new_ch = await self._request(session, f'/guilds/{target_id}/channels', 'POST', payload)
            if new_ch:
                self.channel_id_map[ch['id']] = new_ch['id']
                self.clone_stats['channels'] += 1
                ...
                if clone_webhooks:
                    ...
                    if webhooks:
                        for wh in webhooks:
                            ...
                            await self._request(session, f'/channels/{new_ch["id"]}/webhooks', 'POST', wh_payload)
